...
public class JavaFXFileUploadSample extends Application {
    ...
    private static void displayFileChooserDialog(final FileChooserParams params, final AtomicReference<CloseStatus> result, final BrowserView browserView) {
        final CountDownLatch latch = new CountDownLatch(1);
        Platform.runLater(new Runnable() {
            @Override
            public void run() {
                try {
                    Window window = FXUtil.getWindowForNode(browserView);
                    if (window != null) {
