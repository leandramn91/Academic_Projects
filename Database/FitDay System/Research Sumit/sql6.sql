...
ALTER TABLE `Activity_Table` ADD CONSTRAINT `ComparedWith` FOREIGN KEY (FK) REFERENCES `SafeHeartRate_Table` () ON DELETE RESTRICT ON UPDATE RESTRICT
...
drop foreign key Performs;

alter table DiabeticMeasure_Table
drop foreign key SampleMeasures;

alter table Treatment_Table
drop foreign key Suggests;



alter table UserHistory_Table
drop foreign key UserHistoryMaintained;

alter table PatientHistory_Table
drop foreign key ArchievedIn;

alter table Activity_Table
drop foreign key ConsistsOf;

alter table Remedy_Table
drop foreign key Recommends;

alter table Remedy_Table
drop foreign key ComprisesOf;

alter table Doctor_Table
drop foreign key hasA;

alter table Patient_Table
drop foreign key Relationship2;
